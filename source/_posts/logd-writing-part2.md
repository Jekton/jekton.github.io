---
title: Android log 机制 - logd 如何接收 log 数据（下）
date: 2018-05-17 08:34:22
categories: Android
...
[上一篇](/2018/05/16/logd-writing-part1/)中我们知道，当有数据可读的时候，`SocketListener` 会回调子类的 `onDataAvailable()`。对于 `LogListener` 来说，就是在这个函数里面完成对 log 的读取的。
```C++
